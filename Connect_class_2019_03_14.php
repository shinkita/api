        ...
        if ($mysql = $this->conn->query($query)) {
            ...
            if ($mysql->num_rows > 0) {
                while ($row = $mysql->fetch_assoc()) {
                    ...
                    $que = "SELECT id FROM friends_list WHERE (user_id = '$user_id' OR friends_id = '$user_id') AND (user_id = '$row[user_id]' OR friends_id = '$row[user_id]') AND approved=1 AND profile_type=1";
                    $msql = $this->conn->query($que);
                    $row['is_friend'] = 0;
                    $row['is_faimily'] = 0;
