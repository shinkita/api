...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        if (isset($_POST['name']) && isset($_POST['email']) && isset($_POST['password']) && isset($_POST['dob']) && isset($_POST['gender']) && isset($_POST['pic_friend']) && isset($_POST['pic_family']) && isset($_POST['pic_pro']) && isset($_POST['country_code'])) {
            ...
            $data = array(
                'table' => 'users',
                'field' => 'email,password,activated,mobile,country_code,source',
                'values' => array(array($email, sha1($password), 1, $mobile, $country_code,$source))
            );
        $querylogs = "logs/signup_logs_".date('Ymd').".txt";
		  error_log($data, 3,  $querylogs);
        ...
        $shadow1 = $db->insert(array('table' => 'user_detail', 'field' => 'user_id,name,dob,gender,interest', 'values' => array(array($result['inserted_id'], $name, $dob, $gender,$interest))));
		...
		$email_body="<p>Dear ".$name.",<br>
Welcome to ViaSpot! We are very pleased  you’ve chosen ViaSpot platform for your social media.<br> Enjoy sharing and posting with your friends, family and professionals. </p> 
<p>
Regards<br>
Viaspot Management Team
</p>";
		$email_msg = array(
           ...
           $mail_result= $db->send_mail($email, $email_msg);
		   $maillogs = "logs/welcomeMaillogs_".date('Ymd').".txt";
		   $mail_data=$mail_result."|".$email;
		   error_log($mail_data, 3, $maillogs);
