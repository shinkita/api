...
Class db_connect {
    ...
    public function get_user_data_id($id) {
        try {
            $data_user = array(
                'table' => 'user_detail',
                'select' => 'user_detail.user_id AS id,user_detail.name AS name,(SELECT profile_pic FROM profile_pic WHERE profile_type=1 AND user_id=' . $id . ') AS friend_pic,(SELECT profile_pic FROM profile_pic WHERE profile_type=2 AND user_id=' . $id . ') AS faimily_pic,(SELECT profile_pic FROM profile_pic WHERE profile_type=3 AND user_id=' . $id . ') AS pro_pic',
                'where' => 'user_id=' . $id . ''
            );
